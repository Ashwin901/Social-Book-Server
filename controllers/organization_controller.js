...
OrganizationController.put("/:id", VerifyToken, async (req, res) => {
    try {
        ...
        if (!organization) {
            res.status(404).json("Organization not found!! Please try again");
        }

        await organization.updateOne(updatedOrganization);
